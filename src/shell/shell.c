 ...
 */
static THD_WORKING_AREA(ShellVA, 350);
  ...
  while (true) {
    ...
    if (cmd != NULL) {
      if (strcmp(cmd, "help") == 0) {
        ...
        list_commands(chp, shellCommands);
        if (scp != NULL)
          list_commands(chp, scp);
        chprintf(chp, SHELL_NEWLINE_STR);
      }
      else if (cmdexec(shellCommands, chp, cmd, n, args) &&
 ...
 * @param scfg
 */
void Shell__thread_init(const ShellConfig *scfg){
	thread_t *shelltp = chThdCreateStatic(ShellVA, sizeof(ShellVA), NORMALPRIO + 10, shellThread, (void*)scfg);
	chThdWait(shelltp);
}
